...
- **Static gates enforce the above:** `npm run lint` runs eslint + stylelint + `check:tokens` (no hardcoded colors) + `check:dvfy-pref` (prefer `<dvfy-*>` over native) + `check:taxonomy` (composition law, stratum rules, header drift) + `check:upgrade-order` (attributeChangedCallback cannot dereference unbuilt state — see the bullet above) + `check:brand-assets` (every painted colour in a brand SVG is a token, and its comments are well-formed XML). Each gate has been proven red against a real defect before being trusted green. Tests: `npm run test` (web-test-runner + Playwright). Regenerate the manifest after API changes: `npm run analyze` → `custom-elements.json`. See `docs/conventions.md` for the allowlisting mechanisms.
