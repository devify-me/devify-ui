...
- `scripts/check-taxonomy.mjs` — now also guards **classification drift**: a component's source comments and its `custom-elements.json` description may not state a tier/stratum that contradicts `COMPONENT_REGISTRY`. The drift class shipped twice (#387 "Tier 5 Layout", #388 "Tier 3 organism") and was caught only by human review; the catalog API viewer renders that prose live. Claim parser unit-tested in `scripts/check-taxonomy.test.mjs` (`npm run test:scripts`, new). `check:taxonomy` and `test:scripts` are now wired into CI, which previously ran neither. (#389)
...
- `dvfy-optin`, `dvfy-thank-you` — `heading-level` attribute (`h1` | `h2` | `h3`, default `h1`). The hero heading now renders as an `<h1>` by default, so a pure-widget chum page (shell + optin, or shell + thank-you) has exactly one `<h1>` with no page-authored hidden-h1 hack. Drop to `h2`/`h3` only when the widget isn't the page's top heading. Additive, backward-compatible. (#396)
- `dvfy-chum-page` — `logo-only` boolean attribute. With a `logo` present, renders the logo mark alone and drops the brand-text span (the logo `alt` still carries the brand name, so the accessible name is unchanged). No-op without a logo; default off. Replaces the per-page `.dvfy-chum-page__brand-text{display:none}` BEM-internal workaround. Handled by the shared `utils/no-nav-shell` header builder. (#397)
...
- `dvfy-radio` — no longer destroys and rebuilds its subtree on every attribute change. `#handleChange` set `checked`, which ran `#build()`, which ran `this.textContent = ''` and annihilated the input the user was standing on: `document.activeElement` became `BODY` on every selection. A keyboard or screen-reader user filling the devify.me quiz was ejected from the form each time they chose an option (WCAG 2.4.3 Focus Order, 3.2.2 On Input). `checked`, `disabled` and `label` now update in place, preserving focus, node identity and listeners. (#407)
- `dvfy-auth` — a structural re-render (the in-place sign-in/sign-up toggle, with or without the modal open) no longer drops keyboard focus to `<body>`. Focus is restored to the rebuilt toggle link, or to the first form field, and is left alone when it was never inside the component. `method` is now trimmed, so `method=" get "` normalizes to `get` instead of silently falling back to `post`. (#392)
