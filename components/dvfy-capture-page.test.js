...
import './dvfy-capture-page.js';

describe('dvfy-capture-page', () => {
  it('is defined as a custom element', () => {
    expect(customElements.get('dvfy-capture-page')).to.exist;
  });
  ...
  describe('page scaffold', () => {
    ...
    it('renders exactly one <main> (skip-link target / single conversion path)', async () => {
      const el = await fixture(html`<dvfy-capture-page><p>body</p></dvfy-capture-page>`);
      expect(el.querySelectorAll('main').length).to.equal(1);
    });

    it('is a block-level element', async () => {
      const el = await fixture(html`<dvfy-capture-page>x</dvfy-capture-page>`);
  ...
  describe('1:1 attention ratio — NO navigation menu', () => {
    it('renders zero <dvfy-nav-menu> / <dvfy-nav> escape routes', async () => {
      const el = await fixture(html`
        <dvfy-capture-page brand="Renting Ideal"><dvfy-optin></dvfy-optin></dvfy-capture-page>
      `);
    ...
    it('a plain-text brand is NOT a link', async () => {
      const el = await fixture(html`<dvfy-capture-page brand="Renting Ideal"><p>x</p></dvfy-capture-page>`);
      const brandText = el.querySelector('.dvfy-capture-page__brand-text');
  ...
  describe('optional brand mark', () => {
    it('renders no <header> when neither brand nor logo is set', async () => {
      const el = await fixture(html`<dvfy-capture-page><p>x</p></dvfy-capture-page>`);
    ...
    it('renders a logo <img> with alt text when logo is set', async () => {
      const el = await fixture(html`
        <dvfy-capture-page brand="Renting Ideal" logo="/logo.svg"><p>x</p></dvfy-capture-page>
      `);
  ...
  describe('home-href — single self-referential link only', () => {
    it('wraps the brand in exactly one self-link when home-href is set', async () => {
      const el = await fixture(html`
        <dvfy-capture-page brand="Renting Ideal" home-href="#top"><p>x</p></dvfy-capture-page>
      `);
    ...
    it('sanitizes a hostile home-href down to "#"', async () => {
      const el = await fixture(html`
        <dvfy-capture-page brand="X" home-href="javascript:alert(1)"><p>x</p></dvfy-capture-page>
      `);
  ...
  describe('theming', () => {
    it('brand bar background is bound to a semantic surface token', async () => {
      ...
      const el = await fixture(html`<dvfy-capture-page brand="X"><p>y</p></dvfy-capture-page>`);
  ...
  describe('robustness', () => {
    it('does not double-wrap on reconnection', async () => {
      const el = await fixture(html`<dvfy-capture-page brand="X"><p>once</p></dvfy-capture-page>`);
      el.remove();
